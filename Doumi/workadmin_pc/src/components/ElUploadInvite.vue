  ...
  <div class="uploadWrap" :style="{width:width+'px'}">
    ...
    <div class="uploadSection">
      <el-upload ref="upload" :class="{hide:hideUpload,hide2:isDet}" :action="actionUrl" :data="uploadData" :name="uploadName" :on-preview="preview" :on-remove="handleRemove" :on-success="uploadImage" :before-upload="beforeUpload"
        :on-error="handleUploadError" :file-list="getFileListValue" :limit="limitCount" :on-change="onChange" accept=".png,.gif,.jpg,.jpeg" :disabled="isDisabled">
...
export default {
  ...
  data() {
    return {
      hideUpload: false,
      actionUrl: util.host+'/sea/api/1.0/client/v1/common/upload?dmclient=pcweb&X-Doumi-Agent=weixinqy',
      uploadData: {
        team_id: this.teamid,
  ...
  mounted() {
    console.log('chilllddddddddd',this.projectId);
    // this.uploadData.team_id=this.team_id
  },
  methods: {
    preview(file) {
      ...
      if (status == 'success'&& response) {
        ...
        window.open('https://cdn.doumistatic.com/' + file.url, '_blank');
      }
    },
    beforeUpload(file) {
