  ...
  <div id="kqaddmin">
    <div class="kq-wapper">
      <h2>考勤管理
        <div style="float:right;">
          ...
          <router-link to="kqtasklist" style="color:#6699ee;font-weight:normal;font-size:13px;line-height: 1;"
            class="kaaddminset">考勤规则管理<i class="setico_svg"></i></router-link>
        </div>
      </h2>
      <div class="kq-header">
        <div class="export-dialog">
          ...
          <el-dialog title="考勤导出" :visible.sync="dialogVisible" @close="resetForm('ruleForm')" size="tiny">
            ...
            <div class="kq-select-div">
              <el-form :model="ruleForm" :rules="rules" ref="ruleForm">
                ...
                <el-form-item label="开始日期" :label-width="formLabelWidth" prop="sday">
                  <el-date-picker v-model="ruleForm['sday']" type="date" placeholder="选择日期" style="width: 100%;"
                    :editable="false">
                  </el-date-picker>
                </el-form-item>
                <el-form-item label="结束日期" :label-width="formLabelWidth" prop="eday">
                  <el-date-picker v-model="ruleForm['eday']" type="date" placeholder="选择日期" style="width: 100%;"
                    :editable="false">
                  </el-date-picker>
                </el-form-item>
                <el-form-item v-show='selectIf' label="考勤规则" :label-width="formLabelWidth" prop="kqTask_">
                  <el-select :disabled="selectIfflag" multiple v-model="ruleForm['kqTask_']" @change="selectTaskId_"
                    style="width:100%;">
                    <el-option v-for="item in kqTaskOptions" :key="item.id" :label="item.name" :value="item.id">
                    </el-option>
                  </el-select>
                </el-form-item>
                <el-form-item v-show='!selectIf' label="考勤规则" :label-width="formLabelWidth" prop="kqTask">
                  <el-select :disabled="selectIfflag" v-model="ruleForm['kqTask']" @change="selectTaskId"
                    style="width:100%;" class="mystatus">
                    <el-option v-for="item in kqTaskOptions" :key="item.id" :label="item.name" :value="item.id">
                    </el-option>
                  </el-select>
                </el-form-item>
                  ...
                  <el-form label-width="40px" :rules="rules" :model="ruleForm">
                    <div class="from-item-list">
                      <el-form-item label="日期：" prop="kqDateDay">
                        <el-date-picker v-model="ruleForm.kqDateDay" type="date" placeholder="选择日期" style="width:225px"
                          :editable="false">
                        </el-date-picker>
                      </el-form-item>
                    </div>
                    ...
                    <div class="from-item-list member">
                      <el-form-item label="人员：">
                        <el-autocomplete popper-class="my-autocomplete2" style="width:135px;" v-model="memberDay"
                          :fetch-suggestions="querySearch" custom-item="my-item-zh" :trigger-on-focus="false"
                          placeholder="姓名/手机号" @select="handleSelect">
                        </el-autocomplete>
                      </el-form-item>
                    </div>
                    <div class="from-item-list">
                      <el-form-item label="状态：" style="float:left">
                        <el-select v-model="stateDay" class="mystatus" style="width:106px" @change="selectStateId">
                          <el-option v-for="item in stateOptionsDay" :key="item.value" :label="item.label"
                            :value="item.value">
                          </el-option>
                        </el-select>
                      </el-form-item>
                      <el-button type="primary" class="seach-btn" @click="getTableDataDay(1)"
                        style="float:left:margin-left:16px;">搜 索</el-button>
                    </div>
                  </el-form>

                </div>
                <div class="kq-table-day">
                  <el-table :data="tableDataDay" border style="width: 100%" :height="winHeight" empty-text="暂无数据"
                    :row-class-name="classNameAttribute" @row-click="showKqDetailFn" v-loading.body="loadingDay">
                    <el-table-column prop="name" show-overflow-tooltip label="姓名" width="100"
            ...
            <el-tab-pane label="按月统计" name="按月统计">
              <div class="kq-count-month">
                  ...
                  <el-form label-width="40px" :rules="rules" :model="ruleForm">
                    <div class="from-item-list">
                      <el-form-item label="日期：" prop="kqDateMonth">
                        <el-date-picker v-model="ruleForm.kqDateMonth" type="daterange" style="width:225px;"
                    ...
                    <div class="from-item-list member">
                      <el-form-item label="人员：">
                        <el-autocomplete popper-class="my-autocomplete2" v-model="memberMonth" style="width:135px;"
                          :fetch-suggestions="querySearch" custom-item="my-item-zh" :trigger-on-focus="false"
                          placeholder="姓名/手机号" @select="handleSelect">
                        </el-autocomplete>
                      </el-form-item>
                ...
                <div class="kq-table-month">
                  <el-table :data="tableDataMonth" border style="width: 100%" :height="winHeight" empty-text="暂无数据"
                    @cell-click="cellClick" v-loading.body="loadingM">
                    <el-table-column :fixed="tableDataMonth.length != 0" prop="name" label="姓名" width="100"
                  ...
                  <el-form label-width="40px" :rules="rules" :model="ruleForm">
                    <div class="from-item-list">
                      <el-form-item label="日期：" prop="kqDateRecord">
                        <el-date-picker v-model="ruleForm.kqDateRecord" type="daterange" style="width:225px"
                          :editable="false" placeholder="选择日期范围">
                        </el-date-picker>
                      </el-form-item>
                    </div>
                    ...
                    <div class="from-item-list member">
                      <el-form-item label="人员：">
                        <el-autocomplete popper-class="my-autocomplete2" v-model="memberRecord" style="width:135px;"
                          :fetch-suggestions="querySearch" :trigger-on-focus="false" custom-item="my-item-zh"
                          placeholder="姓名/手机号" @select="handleSelect">
                        </el-autocomplete>
                      </el-form-item>
                    </div>
                    <div class="from-item-list">
                      <el-form-item label="状态：" style="float:left">
                        <el-select v-model="stateRecord" class="mystatus" style="width:105px" placeholder="请选择"
                          @change="selectRecordStateId">
                          <el-option v-for="item in stateOptionsRecord" :key="item.value" :label="item.label"
                            :value="item.value">
                          </el-option>
                        </el-select>
                      </el-form-item>
            ...
            <el-tab-pane label="考勤排行" name="考勤排行">
              <div class="kq-record">
                  ...
                  <el-form label-width="70px">
                    <div class="from-item-list rankingMain">
                      <el-form-item label="周期范围：" prop="kqDateRecord" class="rakinpInput">
                        <el-date-picker v-model="rankingTime" range-separator="至" type="daterange" style="width:220px">
                        </el-date-picker>
                      </el-form-item>
                      <el-form-item label="周期范围：" prop="kqDateRecord" class="rakpingPicker"
                        v-if="attendance_rank_cycle == 2">
                        <el-date-picker @change="weekChange" v-model="rankingWeek" type="week" format="yyyy-MM-dd"
                          style="width:240px">
                        </el-date-picker>
                      </el-form-item>
                      <el-form-item label="周期范围：" prop="kqDateRecord" class="rakpingPicker"
                        v-if="attendance_rank_cycle == 1">
                        <el-date-picker @change="monthChange" v-model="rankingMonth" type="month" format="yyyy-MM-dd"
                          style="width:240px">
                        </el-date-picker>
                      </el-form-item>
                    </div>
                ...
                <div class="kq-table-record kq-tableranking-list">
                  <el-table :data="tableKqRankinglist" border style="width: 100%;overflow-x:hidden;" :height="RkwinHeight"
                    :empty-text="RkEmptyText" v-loading.body="loadingRk">
                  ...
                  <el-pagination v-if="total != 0" @current-change="handleSizeChange" :current-page.sync="Rkpage"
                    :page-size="Rkpage_size" layout="total, prev, pager, next" :total="total">
                  </el-pagination>
                </div>
              </div>
            </el-tab-pane>
            <el-tab-pane label="补卡审批" name="补卡审批" v-if="setReplacementCardApproval">
              <div class="kq-count-day">
                <div class="kq-seach-form">
                  <el-form label-width="40px" :rules="rules" :model="ruleForm">
                    <div class="from-item-list member">
                      <el-form-item label="人员：">
                        <el-autocomplete popper-class="my-autocomplete2" style="width:135px;" v-model="memberDayPushin"
                          :fetch-suggestions="querySearch" custom-item="my-item-zh" :trigger-on-focus="false"
                          placeholder="姓名/手机号" @select="handleSelect">
                        </el-autocomplete>
                      </el-form-item>
                    ...
                    <div class="from-item-list">
                      <el-form-item label="状态：" style="float:left">
                        <el-select v-model="replacementCardApproval" class="mystatus" style="width:106px"
                          @change="selectStateId">
                          <el-option v-for="item in replacementCardApprovalStatus" :key="item.value" :label="item.label"
                            :value="item.value">
                          </el-option>
                        </el-select>
                      </el-form-item>
                      <el-button type="primary" class="seach-btn" @click="getReplacementCardApproval(1)"
                        style="float:left:margin-left:16px;">搜 索</el-button>
                    </div>
                  </el-form>
                ...
                <div class="kq-table-day">
                  <el-table :data="tableReplacementCardApproval" :height="winHeight" empty-text="暂无数据" :row-class-name="classNameAttribute">
                    <el-table-column prop="real_name" show-overflow-tooltip label="姓名" width="120"></el-table-column>
                    ...
                    <el-table-column prop="task_name" label="考勤规则名称" width="220"></el-table-column>
                    <el-table-column label="异常原因" width="220">
                      <template slot-scope="scope">
                        {{scope.row.attendance_status}}
                      </template>
                    </el-table-column>
                    <el-table-column prop="create_at" label="补卡提交时间" width="120"></el-table-column>
                    <el-table-column prop="date" label="补卡日期" width="120"></el-table-column>
                    <el-table-column prop="time" label="补卡时间" width="120"></el-table-column>
                    <el-table-column prop="apply_reason" label="补卡原因" min-width="220"></el-table-column>
                    <el-table-column class-name="className" label="处理状态" width="120">
                      <template slot-scope="scope">{{scope.row.audit_status=='0'?'待审批':scope.row.audit_status=='5'?'已审批':'已拒绝'}}</template>
                    </el-table-column>
                    <el-table-column width="120" fixed="right" label="操作" text-align="center">
                      <template slot-scope="scope">
                        <el-button size="mini" v-if="scope.row.audit_status=='0'" type="primary" @click="examineAndApprove(scope.row)">审批</el-button>
                      </template>
                    </el-table-column>
                  </el-table>
                  <el-pagination v-if="total != 0" @current-change="handleCurrentChangeReplacement"
                    current-page.sync="1" page-size="10" layout="total, prev, pager, next" :total="total">
                  </el-pagination>
                </div>
              </div>
            </el-tab-pane>
    ...
    <div class="kq_detail">
      ...
      <el-dialog title="考勤打卡记录" :visible.sync="showKqDetail" :close-on-press-escape="false">
        <div class="user_info">
          <div v-if="kq_detail.extra_info.user_avatar && kq_detail.extra_info.user_avatar.url" class="avatar"
            :style="'background-image:url(' + kq_detail.extra_info.user_avatar.url + ');'"></div>
          ...
          <div class="right_info">
            ...
            <p class="kq_team">
              ...
              小组：{{ kq_detail.extra_info.group_name ||
                '未知小组' }}&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;所属考勤任务：{{ kq_detail.extra_info.task_name }}
            </p>
          </div>
        </div>
        <div class="kq_info_wrap" v-loading="loading_kq_detail">
          <template v-for="(attendance, index) in kq_detail.attendance_list">
            <div class="kq_info_item">
              ...
              <div class="kq_date">
                <p>{{ attendance.type == 1 ? "上班" : "下班" }}&nbsp;&nbsp;{{ attendance.cross == 1 ? "次日" :
                  "" }}{{ attendance.require_time }}</p>
                <div class="kq_status">
              ...
              <div class="kq_format">
                <p class="attend_time" v-if="attendance.attend_time">{{ attendance.attend_nextday == 1 ? "次日" :
                  "" }}{{ attendance.attend_time }}</p>
                <template v-for="format in attendance.form_data">
                  <div class="format_item">
                    <p class="item_title"
                      v-if="(format.type !== 'Imageview' && format.value) || (format.type === 'Imageview' && format.pic_list.length)">
                      {{ format.title }}</p>

                    <p class="item_info" v-if="format.type === 'Location'">{{ format.value.addr_name }}({{
                      format.value.addr }})
                    </p>
                    <p class="view_location" v-if="format.type === 'Location'">
                      <span
                        @click="viewLocationFn(kq_detail.attendance_list.task_id, attendance.attendance_id)">查看位置</span>
                    </p>
                    <p class="item_info" v-if="format.type !== 'Location' && format.type !== 'Imageview'">{{ format.value
                    }}</p>
                    <div class="pic_box" v-if="format.type === 'Imageview'">
                      <div class="img" v-for="(pic, index) in format.pic_list">
                        <img class="preview-img" :src="pic.msrc" @click="imgPreview(index, format.pic_list)">
                      </div>
                      <div class="clear"></div>
                    </div>
                  </div>
        ...
        <div class="kq_detail_foot">
          <div class="toggle_btn">
            ...
            <el-button v-if="kq_detail_index < cur_data.length - 1" class="hover" icon="arrow-right"
              @click="changeKqDetail(1)"></el-button>
    ...
    <div class="kq_detail_location">
      ...
      <el-dialog
        title="提示"
        :visible.sync="pushInDialogVisible"
        size="tiny">
        <div style="padding: 10px 20px 15px;">是否同意该申请？</div>
        <span slot="footer" class="dialog-footer">
          <el-button @click="pushInDialogVisible = false">取 消</el-button>
          <el-button type="danger" @click="submitApplyFor(-5)">拒 绝</el-button>
          <el-button type="primary" @click="submitApplyFor(5)">确 定</el-button>
        </span>
      </el-dialog>
    </div>
    <!-- 选择小组弹窗 -->
    <select-group-multi ref="profile" :all-groups="all_groups"
      v-on:confirmGroupSelecter="confirmGroupSelecter"></select-group-multi>
...
let map = null;
let marker = null;
let kqdownload1 = false;
let kqdownload2 = true;
...
export default {
  ...
  data: function () {
    ...
    var kqTaskNameTest = (rule, value, callback) => {
      if (!value && !this.selectIf && !this.ruleForm.kqTask.length) {
    ...
    var kqTaskNameTest_ = (rule, value, callback) => {
      if (!value.length && this.selectIf && !this.ruleForm.kqTask_.length) {
        return callback(new Error("请选择考勤规则"));
      } else {
        callback();
      }
    ...
    return {
      ...
      rules: {
        //日期验证规则
        kqDateDay: [{ validator: testStartTime0, trigger: "change" }],
        kqDateMonth: [{ validator: testStartTime, trigger: "change" }],
        kqDateRecord: [{ validator: testStartTime, trigger: "change" }],
        kqTask: [{ validator: kqTaskNameTest, trigger: "blur" }],
        kqTask_: [{ validator: kqTaskNameTest_, trigger: "blur" }],
        kqExport: [{ validator: kqExportType, trigger: "change" }],
        sday: [{ validator: startTime, trigger: "change" }],
        eday: [{ validator: endTime, trigger: "change" }]
      },
      ...
      selectIf: true, //单双选切换,
      selectIfflag: true, // 规则是否可以点击
      kqdownload1: true,
      kqdownload2: false,
      kqdownload3: false,
      // 补卡审批
      setReplacementCardApproval: false, //超管展示补卡审批
      replacementCardApproval: -1, // 补卡审批
      replacementCardApprovalStatus: [
      {
          label: '全部',
          value: -1
        },
    ...
    kqMonthClassArr (val) {
      ...
      setTimeout(() => {
        ...
        this.kqMonthClassArr.forEach(obj => {
          $(".kq-table-month .el-table__body-wrapper .el-table__body tbody tr")
            .eq(obj[0])
            .find("td")
            .eq(obj[1] + 3)
            .css({ color: "#ff6600" });
  ...
  methods: {
    ...
    getTableDataDay (exp) {
      ...
      console.log(this.ruleForm.kqDateDay);
    ...
    getTableDataMonth (exp) {
      this.loadingM = true;
      if (!this.ruleForm.kqDateMonth || !this.ruleForm.kqDateMonth[0]) {
        this.loadingM = false;
        return;
      }
      let sumDays = util.DateDiff(
        util.formatData1(this.ruleForm.kqDateMonth[0]),
        util.formatData1(this.ruleForm.kqDateMonth[1])
      );
      if (sumDays > 31) {
        this.loadingM = false;
        return;
    ...
    getTableDataRecord (exp) {
      this.loadingR = true;
      if (!this.ruleForm.kqDateRecord || !this.ruleForm.kqDateRecord[0]) {
        this.loadingR = false;
        return;
      }
      let sumDays = util.DateDiff(
        util.formatData1(this.ruleForm.kqDateRecord[0]),
        util.formatData1(this.ruleForm.kqDateRecord[1])
      );
      if (sumDays > 31) {
        this.loadingR = false;
        return;
    ...
    // 考勤--补卡审批
    getReplacementCardApproval (exp) {
      if (exp && exp == 1) {
        console.log('vvvv',this.states)
        if (this.states.length != 0) {
          if (
            !this.select_user_id &&
            this.states[0].status == -1 &&
            this.memberDayPushin
          ) {
            this.tableReplacementCardApproval = [];
            return;
          }
          if (!this.memberDayPushin) {
            this.user_id = 0;
          }
          if (this.select_user_id == -1 && this.states[0].status == -1) {
            this.user_id = 0;
      ...
      let _data = {
        ...
        project_id: this.project_id,
        user_id: this.user_id,
        audit_status: this.replacementCardApproval,
        page: this.Rkpage,
        page_size: 10,
      };
      util.ajax({
        url: "/attendance/punch_apply_list",
        type: "GET",
        data: _data,
        timeout: 10000,
        success: obj => {
          // console.log(obj)
          if (obj && obj.errno == 0) {
            this.tableReplacementCardApproval = obj.data.list;
            this.total = parseFloat(obj.data.total);
          }
        },
        error: (xhr, status) => {
          this.loadingR = false;
    ...
    examineAndApprove(row){
      console.log(row)
      this.pushInDialogVisible = true;
      this.pushInRow = row;
      return
      this.$confirm('是否同意该打卡审批?', '提示', {
    ...
    submitApplyFor(type){
      let _data = {
        team_id: this.team_id,
        project_id: this.project_id,
        audit_id:this.pushInRow.id,
        audit_status:type
      };
      util.ajax({
        url: "/attendance/punch_apply_audit",
        type: "POST",
        data: _data,
        timeout: 10000,
        success: obj => {
          // console.log(obj)
          this.pushInDialogVisible = false;
    ...
    alertIsexport () {
      if (this.tab_index != 3) {
        ...
        let downloadUrl =
          "/sea/api/1.0/client/v1/attendance/list?dmclient=pcweb&team_id=" +
          this.team_id +
          "&project_id=" +
          this.project_id +
          "&start=" +
    ...
    downloadUrl () {
      downloadUrl =
        "/sea/api/1.0/client/v1/attendance/attendance/export2?dmclient=pcweb&team_id=" +
        this.team_id +
        "&project_id=" +
        this.project_id +
        "&start_date=" +
        util.formatData1(this.ruleForm.sday) +
        "&end_date=" +
        util.formatData1(this.ruleForm.eday) +
        "&export=" +
        1 +
        "&task_id=" +
        this.task_id;
    ...
    confirm (formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.dialogVisible = false;
          this.kqdownload1 = false;
          this.kqdownload2 = true;
          this.kqdownload3 = false;
          let data_ = {
            team_id: this.team_id,
            project_id: this.project_id,
            start_date: util.formatData1(this.ruleForm.sday),
            end_date: util.formatData1(this.ruleForm.eday),
            task_id: this.selectIf
              ? JSON.stringify(this.ruleForm.kqTask_)
              : `["${this.ruleForm.kqTask}"]`,
            type: this.ruleForm.kqExport // 导出类型参数
          };
          util.ajax({
            ...
            success: obj => {
              if (obj && obj.errno == 0) {
                ...
                util.setLocalStorage(
                  `pollingkey${this.project_id}${util.getLocalStorage('viewtype')}`,
                  key,
                  80000
                );
                let polldata = Object.assign(data_, {
                  key: key,
                  dmclient: "pcweb"
                });
                var count = 0
                  ...
                  util.ajax({
                    ...
                    success: obj => {
                      ...
                      if (obj && obj.errno == 0 && obj.data.download_url) {
                        console.log('轮询成功' + count * 3 + 's')
                        window.clearInterval(trimeId);
                        _this.kqdownload1 = false;
                        _this.kqdownload2 = false;
                        _this.kqdownload3 = true;
                        util.setLocalStorage(
                          `pollingurl${this.project_id}${util.getLocalStorage('viewtype')}`,
                          obj.data.download_url,
                          80000
                        );
                      } else if (count == 60) {
                        console.log('轮询失败' + count * 3 + 's')
                        _this.kqdownload1 = true;
                        _this.kqdownload2 = false;
                        _this.kqdownload3 = false;
                        localStorage.removeItem(`pollingurl${this.project_id}${util.getLocalStorage('viewtype')}`);
                        localStorage.removeItem(`pollingkey${this.project_id}${util.getLocalStorage('viewtype')}`);
                        window.clearInterval(trimeId);
                        this.$message({
                          showClose: true,
                          message: '导出失败，请重新导出',
                ...
                this.kqdownload1 = true;
                this.kqdownload2 = false;
                this.kqdownload3 = false;
                localStorage.removeItem(`pollingurl${this.project_id}${util.getLocalStorage('viewtype')}`);
                localStorage.removeItem(`pollingkey${this.project_id}${util.getLocalStorage('viewtype')}`);
                window.clearInterval(trimeId);
    ...
    kqdownloadfuc () {
      ...
      this.kqdownload2 = false;
      this.kqdownload3 = false;
      let pollingurl = util.getLocalStorage(`pollingurl${this.project_id}${util.getLocalStorage('viewtype')}`);
    ...
    cellClick (row, column, cell, event) {
      ...
      if (
        ...
        if (row.task_id != 0) {
          ...
          if (row.date_data[column.property.split("_")[1]].status) {
            //缺卡也不显示状态  不能点击
    ...
    selectTaskId (value) {
      this.ruleForm.kqTask = value
    },
    selectTaskId_ (value) {
      this.ruleForm.kqTask_ = value
    },
    // 导出类型
    selectkqExport (value) {
      this.selectIfflag = false;
      if (value == "3") {
        this.selectIf = false;
      } else {
        this.selectIf = true;
      }
      this.ruleForm.kqExport = value;
    },
    getRankingCycle () {
      this.loadingRk = true;
      util.ajax({
    ...
    handleCurrentChangeReplacement(e){
      console.log('afsfsfdf',e)
      this.Rkpage =e;
      this.getReplacementCardApproval()
    },
    ...
    viewMap (data) {
      ...
      //   icon: p_icon2,
      //   position: [
      //     data.attend_position.coordinate.split(",")[0],
      //     data.attend_position.coordinate.split(",")[1]
      //   ],
      ...
      //     icon: p_icon1,
      //     position: [
      //       obj.coordinate.split(",")[0],
      //       obj.coordinate.split(",")[1]
      //     ],
      ...
      //打卡位置
      let pt = new BMap.Point(data.attend_position.coordinate.split(",")[0], data.attend_position.coordinate.split(",")[1]);
      marker = new BMap.Marker(pt, {
        // icon: p_icon2,
        // offset: new BMap.Pixel(-46, -84),
        title: '打卡位置',
      });
      map.addOverlay(marker);
      let label = new BMap.Label('打卡位置', { offset: new BMap.Size(-18, -22) });
      label.setStyle({
        background: '#FFF',
        padding: '3px',
        fontWeight: 'bold',
      ...
      util.ajax({
        ...
        success: obj => {
          if (obj && obj.errno == 0) {
            ...
            this.setReplacementCardApproval = true
            obj.data.forEach(o => {
  ...
  created () {
    ...
    // todo   判断定时器有没有清除
    let pollingurl = util.getLocalStorage(`pollingurl${this.project_id}${util.getLocalStorage('viewtype')}`);
    let pollingkey = util.getLocalStorage(`pollingkey${this.project_id}${util.getLocalStorage('viewtype')}`);
    if (pollingkey && !pollingurl && trimeId) {
      // 正在生成链接

      this.kqdownload1 = kqdownload1;
      this.kqdownload2 = kqdownload2;
      this.kqdownload3 = kqdownload3;
    } else if (pollingkey && pollingurl) {
      // 已经生成链接
      this.kqdownload1 = false;
      this.kqdownload2 = false;
      this.kqdownload3 = true;
    } else {
      // 生成链接失败
      this.kqdownload1 = true;
      this.kqdownload2 = false;
      ...
      if ($(this).scrollTop() + 150 > that.winHeight / 2) {
        ...
        if (t == true) {
          ...
          if (that.tab_index == 1) {
            ...
            sumDays = util.DateDiff(
              util.formatData1(that.ruleForm.kqDateMonth[0]),
              util.formatData1(that.ruleForm.kqDateMonth[1])
            );
            ...
            sumDays = util.DateDiff(
              util.formatData1(that.ruleForm.kqDateRecord[0]),
              util.formatData1(that.ruleForm.kqDateRecord[1])
            );
