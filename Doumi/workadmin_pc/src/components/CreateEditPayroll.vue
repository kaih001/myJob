		...
		<div class="kq-wapper">
			...
			<div class="kq-header">
				<div class="kq-export" v-if="!ifConfirmPayroll">
					<h3>
						<span @click="improtWages" v-if="!locatData.pay_type == 1"><i class="import-icon"></i>&nbsp;导入工资单</span>
						<span @click="addPersonnel"><i class="personnel-icon-plus"></i>&nbsp;添加人员</span>
					</h3>
				</div>
				<div class="kq-tab">
					<div class="payroll-delEidt editall-btn"  v-if="!ifConfirmPayroll || locatData.pay_type == 1">
						<span class="operationInfo">批量操作：</span>
						<el-button type="primary"  class="yf-defalut " v-bind:class="editsetlist.length === 0 ? 'disablebtnedit' : 'yf-editblue' "  @click="eidtsetpayroll" v-if="!locatData.pay_type == 1">编辑
						</el-button>
					...
					<div class="kq-count-day" v-loading="loading">
						<div class="kq-table-day yf-payrolllist">
							<div class="table_main">
								<div class="table1 project_type3_table" v-if="locatData.project_type&&locatData.project_type==3" style="width: calc(100% - 200px);overflow:auto;margin-bottom:20px;">
									<table class="payroll_table " cellpadding="0" cellspacing="0">
								<!-- <div class="table1" v-if="locatData.project_type&&locatData.project_type==3" :style="{left:-tableLeft + 'px'}" style="overflow:auto"> -->
										...
										<thead class="payroll_table_head">
												<tr>
													<th class="head_user_name">
														<span class="el-checkbox__input" :class="[sel_all ? 'is-checked' : '']" @click="selectAll()"><span class="el-checkbox__inner"></span></span><p>姓名</p>
													</th>
													<th class="source_work" v-if="is_nx_project"><p>人员来源</p></th>
													<th class="head_mobile"><p>手机号</p></th>
													<th class="head_mobile"><p>身份证号</p></th>
													<th class="head_base_wage"><p>实际出勤</p></th>
													<th class="head_base_wage"><p>加班天数</p></th>
													<th class="head_base_wage"><p>工资</p></th>
													<th class="head_base_wage"><p>加班费</p></th>
													<th class="head_base_wage"><p>绩效</p></th>
													<th class="head_base_wage"><p>奖金</p></th>
													<th class="head_base_wage"><p>福利</p></th>
													<th class="head_base_wage"><p>补贴</p></th>
													<th class="head_base_wage"><p>扣费</p></th>
													<th class="head_base_wage"><p>薪酬其他</p></th>
													<th class="head_base_wage"><p>个税</p></th>
													<th class="head_base_wage"><p>五险一金（个人）</p></th>
													<th class="head_base_wage"><p>实发工资</p></th>
                          <th class="head_evaluation"><p>合同状态</p></th>
								...
								<div class="table1" v-else :style="{left:-tableLeft + 'px'}">
                      ...
                      <col width="110px">
											...
											<thead class="payroll_table_head">
												<tr>
													...
													<th class="source_work" v-if="is_nx_project"><p>人员来源</p></th>
													<th class="head_mobile"><p>手机</p></th>
													<th class="head_cq"><p>{{payrollData.base_wage_unit == 0 ? '出勤天数' : '出勤工时' }}</p></th>
													<th class="head_base_wage" v-if="locatData.pay_type"><p>奖金</p></th>
													<th class="head_base_wage" v-else><p>基本工资(元)</p></th>
								...
								<div class="table2" :style="{height:tableHeight}" @scroll="tableScroll()" v-show="memberDataList.length > 0">
									<table class="payroll_table " cellpadding="0" cellspacing="0" :style="{width:910+ 100 *payrollTheadListList.length + 'px' }" >
										<colgroup>
											...
											<col width="100px">
											<col width="90px">
											<col width="90px" v-for="(item,index) in payrollTheadListList" :key="index" >
											<col width="110px">
                      <col width="110px">
											<col width="170px">
											<col min-width="160px">
											...
											<tbody class="payroll_table_body">
												<tr v-for="(item ,index) in memberDataList">
													...
													<td class="source_work" v-if="is_nx_project"><p>{{item.worker_source}}</p></td>
                          ...
                          <td class="body_total_wage"><p>
                            ...
                            >未下发</el-tag>
													...
													<td class="body_evaluation">
														<div  class="evaluation-div" style="width:130px;">
					...
					<el-dialog
						...
						<el-form :model="modifyPayrollform" label-width="97px" :rules="rules" ref="modifyPayrollform">
							...
							<el-form-item label="类型" prop="order_type">
								<el-select v-model="modifyPayrollform.order_type" auto-complete="off" placeholder="请选择类型" style="width: 292px">
									<el-option v-for="item in modifyPayrollform.type_list" :label="item.name" :key="item.id" :value="item.id"></el-option>
								</el-select>
								<p style="color:red;font-size:12px;line-height:16px" v-if="modifyPayrollform.order_type === 1">
									请备注里填写谁代领了谁的工资，金额多少
								</p>
							</el-form-item>
							...
							<el-form-item label="审批人" class="lastmodifyPayroll" prop="approve_uid">
								<el-select v-model="modifyPayrollform.approve_uid" placeholder="请选择" style="width:295px" disabled>
							...
							<el-form-item label="关联工资单" prop="relationPayroll">
								<el-select v-model="modifyPayrollform.relationPayroll" placeholder="请关联对应撤回金额的工资单" style="width:292px" filterable>
									<el-option v-for="item in modifyPayrollform.relationPayrollList" :label="item.name+'-'+item.total_wage/100" :key="item.id" :value="item.id"></el-option>
								</el-select>
							</el-form-item>
							<el-form-item label="备注" prop="remarks">
								<el-input type="textarea" :rows="3" maxlength="200" placeholder="请输入备注" v-model="modifyPayrollform.remarks">
								</el-input>
							</el-form-item>
							<el-form-item label="附件" prop="contractFile">
								<elUpload ref="uploadRef" @getNewFileListFun="getNewFileListFun" :fileListValue="modifyPayrollform.contractFile"></elUpload>
							</el-form-item>
		...
		<div class="fixedbar" >
			<div class="payroll-scope">
				...
				<div class="submit payroll-scoperight">
					...
					<p v-if="this.approve_uid > 0 || this.approve_uid < 0 || locatData.pay_type == 1" @click="submitaudit(1)">提交审核</p>
				</div>
			</div>
		</div>
	</div>
		...
		export default{
			...
			data(){
				...
				const validatorFile = function (rule, value, callback) {
					if (value.length == 0) {
						...
						// if (value.length > 0) {
						//   self.$refs.payrollform.clearValidate('payrollform.contractFile');
						// }
						callback();
					}
				};
				return{
					...
					pay_flag:0,//表示具有直接支付的权限（现在只有超级管理员）
					approve_uid:0,//审批人
					audit_flag:0,//1: 表示登陆者为管理员和超级管理员，0:登陆者为小组长
					jiangjin:0,
					infomsg:'',
			...
			methods:{
				init (){
					...
					// if(util.getLocalStorage('savePayDetail') && util.getLocalStorage('savePayDetail').type == 1){
					// 	this.locatData.pay_type = util.getLocalStorage('savePayDetail').type
					// }
					if(!this.locatData){
					...
					if(memberDataList){
						...
						//console.log(JSON.stringify(memberDataList))
					}else{
              ...
              worker_source:memberList[n].worker_source,
				...
				//批量编辑
				eidtsetpayroll(){
					console.log(this.editsetlist, 'this.editsetlist=====')
				...
				modifyPayrollFn(){
					...
					// 增加关联工资单备注附件展示
					this.modifyPayrollform.remarks = this.locatData.remarks;
					this.modifyPayrollform.contractFile = this.locatData.contractFile;
					this.modifyPayrollform.relationPayroll = this.locatData.relationPayroll;
					this.modifyPayrollform.relationPayrollList = this.locatData.relationPayrollList;
					this.modifyPayrollform.order_name = this.locatData.order_name;
					this.modifyPayrollform.order_type = this.locatData.order_type;
				},
				//获取当前用户权限
				getUserPermission(){
					util.ajax({
						...
						success:(result) => {
							if(result&&result.errno == 0){
								...
								this.modifyPayrollform.type_list = result.data.type_list;
								this.modifyPayrollform.order_name = this.locatData.order_name;
								this.modifyPayrollform.kqDateDay = [this.locatData.start_date,this.locatData.end_date]
							}else{
				...
				modifyPayrollconfirm(formName){
					this.$refs[formName].validate((valid) => {
						if (valid) {
							...
							// 确定修改工资单
							this.locatData.relationPayroll = this.modifyPayrollform.relationPayroll;
							this.locatData.remarks = this.modifyPayrollform.remarks;
							this.locatData.contractFile = this.modifyPayrollform.contractFile;
							this.locatData.relationPayrollList = this.modifyPayrollform.relationPayrollList;
							this.locatData.order_type = this.modifyPayrollform.order_type;
				...
				submitaudit(status_no){
					if(this.selectTotalPerson.length == 0){
				...
				//保存草稿
				TheDraft(status_no){
					console.log('this.selectTotalPerson===', this.selectTotalPerson);
                ...
                protocol_status:memberDataList[i].protocol_status,
					...
					submitObj.type = this.locatData.pay_type == 1 ? 1 : 0;
					submitObj.encourage_rule_id = this.locatData.pay_type == 1 ? this.locatData.encourage_rule_id : '';
					if(this.approve_uid < 0) {
						submitObj.wage_order_id = this.locatData.relationPayroll;
						submitObj.remarks = this.locatData.remarks;
						submitObj.wage_type = this.locatData.order_type;
						submitObj.enclosure = JSON.stringify(this.locatData.contractFile);
					}
					return submitObj;
				},
				//工资单整体数据提交
