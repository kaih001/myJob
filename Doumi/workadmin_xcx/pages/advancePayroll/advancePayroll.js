...
Page({
  ...
  onShow: function () {
    ...
    console.log('onShow..............');
    const biz_id_wage = wx.getStorageSync('biz_id_wage')[0]
    console.log('biz_id_wage>>>', biz_id_wage);
    if (biz_id_wage && JSON.stringify(biz_id_wage) != '{}') {
      this.submitSalaryAdvanceFace();
      return
    }
    this.onInit();
  },
  ...
  submitSalaryAdvance () {
    ...
    // 1.判断是否是会员
    isMember(this.selectedItem).then(({ isMember, goodsId }) => {
      console.log('eeeeeeeeeee', goodsId)
  ...
  submitApply () {
    ...
    console.log('eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee',params)
    submitApply(params).then(apply_id => {
      ...
      setTimeout(() => {
        wx.removeStorageSync('biz_id_wage')
        wx.navigateTo({
  ...
  submitWxPay () {
    ...
    // })
    // return
    const { project_id, team_id, goods_amount, service_fee } = this.selectedItem;
    const params = {
      project_id,
      team_id,
      goods_id: this.goodsId,
  ...
  goFace () {
    wx.navigateTo({
      url: "/pages/soterAuthentication/soterAuthentication?from=wage",
      success: (result) => { },
      fail: () => { },
      complete: () => { },
    });
  ...
  handleInitListData (listData) {
    ...
    const { totalNums, totalServiceFee,totalInterest } = this.handleAmount(item);
  ...
  handleAmount (item) {
    ...
    const service_fee = item.service_fee;
    const interest = item.interest;
    const totalNums = util.formatAmount(amount_total, 2);
    const totalServiceFee = util.formatAmount(service_fee, 2);
    const totalInterest = util.formatAmount(interest, 2);
